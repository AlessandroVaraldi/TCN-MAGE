    ...
    with open(header_path, 'w') as f:
        ...
        for key, data in data_dict.items():
            ...
            for i, value in enumerate(flat_data):
                if i % 8 == 0 and i > 0:  # 8 valori per riga per leggibilità
                    f.write("\n    ")
                elif i == 0:
                    f.write("    ")
                f.write(f"{value}")
                if i < len(flat_data) - 1:  # Evita la virgola dopo l'ultimo elemento
                    f.write(", ")
            f.write("\n};\n\n")
...
def export_reference_input_output(input_data, output_data, header_path, dtype):
    """Genera un unico file .h contenente input e output di riferimento."""
    with open(header_path, 'w') as f:
        ...
        for i, value in enumerate(flat_input):
            if i % 8 == 0 and i > 0:
                ...
                f.write("    ")
            f.write(f"{value}")
            if i < len(flat_input) - 1:  # Evita la virgola dopo l'ultimo elemento
                f.write(", ")
        f.write("\n};\n\n")
        ...
        for i, value in enumerate(flat_output):
            if i % 8 == 0 and i > 0:
                ...
                f.write("    ")
            f.write(f"{value}")
            if i < len(flat_output) - 1:  # Evita la virgola dopo l'ultimo elemento
                f.write(", ")
        f.write("\n};\n\n")
...
def export_tcn_weights_and_refs(model, input_data, output_data, output_dir, scales):
    """Esporta i pesi e gli input/output di riferimento in formato header."""
    ...
    for dtype, dtype_name in dtypes:
        ...
        # Genera un singolo header file per i pesi
...
if os.path.exists(best_model_path) and not force_train:
    ...
    tcn_model, _, _ = train_model(tcn_model, train_loader, val_loader, epochs=1000, patience=100, learning_rate=1e-3)
