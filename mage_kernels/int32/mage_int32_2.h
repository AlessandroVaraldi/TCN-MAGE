...
void mage_l2_tile();
void mage_l2(uint32_t * input_start_addr, uint32_t * outputs_start_addr, uint32_t * weights_start_addr, int time_length, int kernel_size, int input_dim, int output_dim);
...
void mage_l2(uint32_t * input_start_addr, uint32_t * outputs_start_addr, uint32_t * weights_start_addr, int time_length, int kernel_size, int input_dim, int output_dim){
    /* 
        Number of inputs = 128*32=4096 which does not fit in the Mage memory space dedicated to inputs
