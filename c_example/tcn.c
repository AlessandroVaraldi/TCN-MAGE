...
TCNModel *tcn_init(int input_dim, int output_dim, int hidden_dim, int num_layers, int kernel_size) {
    ...
    model->layers = (CausalConv1dLayer *)malloc(sizeof(CausalConv1dLayer)*num_layers);
    ...
    for (int i = 1; i < num_layers - 1; i++) {
        int dilation = 1 << i; // 2^i
        model->layers[i].in_channels = hidden_dim;
        model->layers[i].out_channels = hidden_dim;
    ...
    // Ultimo layer: hidden_dim -> output_dim
    {
        int i = num_layers - 1;
        int dilation = 1 << (num_layers - 1);
        model->layers[i].in_channels = hidden_dim;
        model->layers[i].out_channels = output_dim;
        model->layers[i].kernel_size = kernel_size;
        model->layers[i].dilation = dilation;
        model->layers[i].weight = (float *)malloc(sizeof(float)*output_dim*hidden_dim*kernel_size);
        model->layers[i].bias = (float *)malloc(sizeof(float)*output_dim);
    }
...
void tcn_forward(TCNModel *model, const float *input, float *output, int batch, int time_length) {
    ...
    int max_channels = (model->hidden_dim > model->output_dim) ? model->hidden_dim : model->output_dim;
    ...
    for (int i = 0; i < model->num_layers; i++) {
        ...
        causal_conv1d_forward(&model->layers[i], buffer_in, buffer_out, batch, in_ch, time_length);
