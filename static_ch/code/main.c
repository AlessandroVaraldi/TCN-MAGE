#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <math.h>
#include <string.h>

// Define precision types
#define PRECISION_FLOAT32 1
#define PRECISION_INT8 2
#define PRECISION_INT16 3
#define PRECISION_INT32 4

// === Parametri della rete definiti con #define ===
#define INPUT_DIM 4
#define OUTPUT_DIM 1
#define HIDDEN_DIM 2

// Macro per trovare il massimo di due valori
#define MAX(a, b) ((a) > (b) ? (a) : (b))

// Macro per trovare il massimo di tre valori
#define MAX_OF_THREE(a, b, c) (MAX(MAX((a), (b)), (c)))

const int MAX_DIM = MAX_OF_THREE(INPUT_DIM, OUTPUT_DIM, HIDDEN_DIM);

#define NUM_LAYERS 3
#define KERNEL_SIZE 3
#define BATCH_SIZE 1
#define TIME_LENGTH 8
#define PRECISION PRECISION_INT16

#if defined(PRECISION) && PRECISION == PRECISION_FLOAT32
#include "tcn_weights_float32.h"
#include "tcn_input_output_float32.h"
typedef float dtype;
#elif defined(PRECISION) && PRECISION == PRECISION_INT8
#include "tcn_weights_int8.h"
#include "tcn_input_output_int8.h"
typedef int8_t dtype;
#elif defined(PRECISION) && PRECISION == PRECISION_INT16
#include "tcn_weights_int16.h"
#include "tcn_input_output_int16.h"
typedef int16_t dtype;
#elif defined(PRECISION) && PRECISION == PRECISION_INT32
#include "tcn_weights_int32.h"
#include "tcn_input_output_int32.h"
typedef int32_t dtype;
#else
#error "Tipo di precisione non supportato!"
#endif

// === Buffer statici per input, output e intermedi ===
static dtype input[BATCH_SIZE][INPUT_DIM][TIME_LENGTH];
static dtype output[BATCH_SIZE][OUTPUT_DIM];
static dtype intermediate[BATCH_SIZE][HIDDEN_DIM][TIME_LENGTH];

static int scale;

// Funzione per inizializzare l'input
void initialize_input() {
    for (int i = 0; i < BATCH_SIZE; ++i) {
        for (int j = 0; j < INPUT_DIM; ++j) {
            for (int k = 0; k < TIME_LENGTH; ++k) {
                input[i][j][k] = INPUT_REF[i * INPUT_DIM * TIME_LENGTH + j * TIME_LENGTH + k];
            }
        }
    }
}

// Stampare l'input
void print_input() {
    printf("Input:\n");
    for (int i = 0; i < BATCH_SIZE; ++i) {
        for (int j = 0; j < INPUT_DIM; ++j) {
            for (int k = 0; k < TIME_LENGTH; ++k) {
                printf("%f ", (float)input[i][j][k] / scale);
            }
            printf("\n");
        }
    }
}

// Funzione per ottenere il fattore di scala
void set_scale() {
    if (PRECISION == PRECISION_FLOAT32) {
        scale = 1;
    } else if (PRECISION == PRECISION_INT8) {
        scale = pow(2, 4);
    } else if (PRECISION == PRECISION_INT16) {
        scale = pow(2, 8);
    } else if (PRECISION == PRECISION_INT32) {
        scale = pow(2, 16);
    } else {
        scale = 1;
    }
}

void inference() {
    // Creazione di due buffer intermedi
    static dtype intermediate_a[BATCH_SIZE][HIDDEN_DIM][TIME_LENGTH];
    static dtype intermediate_b[BATCH_SIZE][HIDDEN_DIM][TIME_LENGTH];

    for (int layer = 0; layer < NUM_LAYERS; layer++) {
        int dilation = (layer == 0) ? 1 : (1 << layer);
        int kernel_half = KERNEL_SIZE / 2;

        const dtype *current_weights;
        const dtype *current_bias;

        int input_dim = (layer == 0) ? INPUT_DIM : HIDDEN_DIM;
        int output_dim = (layer == NUM_LAYERS - 1) ? OUTPUT_DIM : HIDDEN_DIM;

        // Scegliere i pesi e i bias del layer
        if (layer == 0) {
            current_weights = (const dtype *)layer0_weights;
            current_bias = layer0_biases;
        } else if (layer == NUM_LAYERS - 1) {
            current_weights = (const dtype *)layer2_weights;
            current_bias = layer2_biases;
        } else {
            current_weights = (const dtype *)layer1_weights;
            current_bias = layer1_biases;
        }

        // Selezionare i buffer da usare per lettura e scrittura
        dtype (*input_buffer)[HIDDEN_DIM][TIME_LENGTH] = (layer == 0) ? (dtype (*)[HIDDEN_DIM][TIME_LENGTH])input : (layer % 2 == 0) ? intermediate_a : intermediate_b;
        dtype (*output_buffer)[HIDDEN_DIM][TIME_LENGTH] = (layer % 2 == 0) ? intermediate_b : intermediate_a;

        // Debugging: Print intermediate inputs
        if (layer != 0) {
            printf("Input layer %d:\n", layer);
            for (int i = 0; i < BATCH_SIZE; ++i) {
                for (int j = 0; j < HIDDEN_DIM; ++j) {
                    for (int k = 0; k < TIME_LENGTH; ++k) {
                        printf("%f ", (float)input_buffer[i][j][k] / scale);
                    }
                    printf("\n");
                }
            }
        }

        for (int batch = 0; batch < BATCH_SIZE; batch++) {
            for (int time = 0; time < TIME_LENGTH; time++) {
                for (int o = 0; o < output_dim; o++) {
                    dtype result = 0;

                    for (int i = 0; i < input_dim; i++) {
                        for (int k = 0; k < KERNEL_SIZE; k++) {
                            int weight_index = o * input_dim * KERNEL_SIZE + i * KERNEL_SIZE + k;
                            int input_time = time - (KERNEL_SIZE - 1 - k) * dilation;

                            if (input_time >= 0) {
                                dtype input_value = input_buffer[batch][i][input_time];
                                result += current_weights[weight_index] * input_value / scale;
                            }
                        }
                    }
                    result += current_bias[o];

                    // Apply ReLU activation if not the last layer
                    if (layer != NUM_LAYERS - 1 && result < 0) {
                        result = 0;
                    }

                    // Save result to the correct buffer
                    if (layer == NUM_LAYERS - 1) {
                        output[batch][o] = result;
                    } else {
                        output_buffer[batch][o][time] = result;
                    }
                }
            }
        }

        // Debugging: Print intermediate outputs
        if (layer != NUM_LAYERS - 1) {
            printf("Output layer %d:\n", layer);
            for (int i = 0; i < BATCH_SIZE; ++i) {
                for (int j = 0; j < HIDDEN_DIM; ++j) {
                    for (int k = 0; k < TIME_LENGTH; ++k) {
                        printf("%f ", (float)output_buffer[i][j][k] / scale);
                    }
                    printf("\n");
                }
            }
        }
    }
}


// Funzione per confrontare l'output
void compare_output() {
    dtype reference_output[BATCH_SIZE][OUTPUT_DIM];
    for (int i = 0; i < BATCH_SIZE; ++i) {
        for (int j = 0; j < OUTPUT_DIM; ++j) {
            reference_output[i][j] = OUTPUT_REF[i * OUTPUT_DIM + j];
        }
    }

    for (int b = 0; b < BATCH_SIZE; ++b) {
        for (int o = 0; o < OUTPUT_DIM; ++o) {
            printf("Output C: %f, Output Python: %f\n", (float)output[b][o] / scale, (float)reference_output[b][o] / scale);
            if (fabs((float)output[b][o] - ((float)reference_output[b][o] / scale)) > 1e-4) {
                printf("Discrepanza trovata!\n");
                return;
            }
        }
    }
    printf("I risultati coincidono!\n");
}

int main() {
    set_scale();

    printf("Inizializzazione dell'input...\n");
    initialize_input();

    printf("Esecuzione dell'inferenza...\n");
    inference();

    printf("Confronto con output di riferimento...\n");
    compare_output();

    return 0;
}
